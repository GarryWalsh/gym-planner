...
:root{
  /* Day expanders — warm amber family around #ffd971 */
  --day-expander-bg:        #fffae6; /* very light amber wash */
  --day-expander-hdr:       #ffe7a3; /* soft header */
  --day-expander-hdr-open:  #ffd971; /* main accent */
  --day-expander-border:    #ffc857; /* slightly deeper border */
  --day-expander-text:      #1f2937; /* slate-800 for strong readability on light */
}

/* Dark mode overrides for day expanders (keep text readable) */
@media (prefers-color-scheme: dark){
  :root{
    --day-expander-bg:       #2b2305; /* deep warm base */
    --day-expander-hdr:      #3a2e07; 
    --day-expander-hdr-open: #4a3a0a;
    --day-expander-border:   #ffd971; /* keep amber pop on border */
    --day-expander-text:     #f8fafc; /* near-white text on dark */
  }
}

/* ── DAY expanders: only siblings AFTER the #day-expanders marker in the SAME block ── */
div[data-testid="stVerticalBlock"]
  > div:has(#day-expanders)
  ~ div > div[data-testid="stExpander"] > details{
    border:1px solid var(--day-expander-border) !important;
    background:var(--day-expander-bg) !important;
    color: var(--day-expander-text) !important; /* body text readable */
}

div[data-testid="stVerticalBlock"]
  > div:has(#day-expanders)
  ~ div > div[data-testid="stExpander"] > details > summary{
    background:var(--day-expander-hdr) !important;
    color: var(--day-expander-text) !important;  /* header text readable */
    padding:.65rem .9rem !important;
}

div[data-testid="stVerticalBlock"]
  > div:has(#day-expanders)
  ~ div > div[data-testid="stExpander"] > details[open] > summary{
    background:var(--day-expander-hdr-open) !important;
}

/* Make sure exercise title links inside the expander adopt the readable color */
div[data-testid="stVerticalBlock"]
  > div:has(#day-expanders)
  ~ div a.exlink{
    color: var(--day-expander-text) !important;
}
...
if plan is None:
    ...
    with hdr_r:
        ...
        badge = "🧠 Generated by AI" if _src == "llm" else "🛠️ Engine: Local"
    ...
    with st.container():
        ...
        with pop_cols[1]:
            with st.popover("🧠 Summary"):
                ...
                # Badge to indicate whether the summary came from the AI or local fallback
                _sum_badge = "🧠 Generated by AI" if summary_llm else "📝 Generated locally"
    ...
    # =========================
    plan_scope = st.container()
    with plan_scope:
        st.markdown('<span id="day-expanders"></span>', unsafe_allow_html=True)
        for day in plan.days:
            with st.expander(f"Day {day.day_index + 1}: {day.label}", expanded=False):
...
with ai_scope:
    st.markdown('<span id="ai-insight-scope"></span>', unsafe_allow_html=True)
    with st.expander("AI Insight", expanded=False):
        if groq_debug.LAST_REQUEST:
            st.markdown(f"**Model:** `{(groq_debug.LAST_USED_MODEL or 'unknown')}`")
            ...
            st.caption("No LLM request recorded yet in this session.")
